...
export default function Home() {
  ...
  const [menuOpen, setMenuOpen] = useState(false); // Menü state
  ...
  console.log("Filtered count:", filteredArticles.length);
  ...
  <div className="max-w-7xl mx-auto px-4 py-3">
    {/* Üst Kısım: Başlık + Hamburger + Arama */}
    <div className="flex items-center justify-between gap-3 mb-3">
      ...
      {/* Hamburger Menü (Sadece Mobil) - SAĞ */}
      <button
      ...
      {/* Arama Çubuğu (Desktop her zaman görünür) */}
      <input
        type="text"
        placeholder="Search..."
        value={searchTerm}
        onChange={(e) => setSearchTerm(e.target.value)}
        className="hidden md:block w-64 px-4 py-2 rounded-lg placeholder-white text-white mt-2 focus:outline-none focus:ring-2 focus:ring-white"
      />
    </div>
    
    {/* Kategori Butonları - Desktop: Her zaman görünür, Mobile: Hamburger açıkken */}
    <div className={`${menuOpen ? 'block' : 'hidden'} md:flex flex-col md:flex-row gap-3`}>
      ...
      <input
        ...
        value={searchTerm}
        onChange={(e) => setSearchTerm(e.target.value)}
        className="md:hidden w-full px-4 py-2 rounded-lg placeholder-white text-white mb-3 focus:outline-none focus:ring-2 focus:ring-white"
      />
      ...
      <div className="flex flex-wrap gap-2">
        {categories.map((cat) => (
          <button
            ...
            onClick={() => {
              setCategory(cat);
              setMenuOpen(false); // Kategori seçince menüyü kapat (mobilde)
            }}
